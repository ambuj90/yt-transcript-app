...
@api_view(["GET"])
def get_transcript(request):
    """Handle multiple video transcript API requests"""
    video_urls = request.GET.get("videoUrl")

    if not video_urls:
        return JsonResponse({"error": "Video URL is required"}, status=400)

    # ✅ Fix issue: Properly split and clean multiple URLs
    ...
    for url in video_urls:
        ...
        transcript_results[url] = transcript if transcript else "Transcript not available"

    return JsonResponse(transcript_results)
